...
**SAVE_TO_OWN**: `ACTIVE → COMPLETED → RELEASED` (+ `CANCELLED`). Inventory item is `RESERVE`d (stock movement) at contract creation, held through `ACTIVE`/`COMPLETED`, and only gets an `ISSUE` movement (status → `ISSUED`) on the explicit release action once `balance_minor = 0`. No deposit concept for this type — customer pays toward the full price from zero.
