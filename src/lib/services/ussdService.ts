 ...
 */
async function beginForCustomer(sessionId: string, dialedMsisdn: string, customerId: string): Promise<UssdResult> {
...
async function startSession(sessionId: string, msisdn: string): Promise<UssdResult> {
  ...
  if (customer) return beginForCustomer(sessionId, msisdn, customer.id);
...
export async function handleUssdInput(params: {
  ...
  switch (existing.state) {
    case 'ENTER_ALT_PHONE': {
      ...
      return beginForCustomer(params.sessionId, params.msisdn, customer.id);
    }
